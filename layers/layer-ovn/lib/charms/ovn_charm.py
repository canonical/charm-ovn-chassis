...
class BaseOVNChassisCharm(charms_openstack.charm.OpenStackCharm):
    ...
    def configure_ovs_dpdk(self):
        ...
        for row in opvs:
            for k, v in (('dpdk-lcore-mask', dpdk_context.cpu_mask()),
                         ...
                         ('pmd-cpu-mask', dpdk_context.pmd_cpu_mask()),
                         ):
    ...
    def custom_assess_status_last_check(self):
        """Override parent method to check for dpdk-lcore-mask and
        pmd-cpu-mask overlap in case OVS DPDK is used.

        Returns:
        blocked, message - if overlap is detected between
        dpdk-lcore-mask and pmd-cpu-maskwhen when using OVS DPDK.
        None, None otherwise.
        """
        status = None
        message = None
